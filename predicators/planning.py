...
def _update_sas_file_with_failure(
    discovered_failure: _DiscoveredFailure, sas_file: str
) -> None:
    """Update the given set of ground_nsrts based on the given
    DiscoveredFailure.
    ...
    (because _GroundNSRTs are frozen).
    """
    ground_op_str = discovered_failure.failing_nsrt.name.lower() + " " +  " ".join(o.name for o in discovered_failure.failing_nsrt.objects)
    ...
    for obj in discovered_failure.env_failure.info["offending_objects"]:
        ...
        # The following range lines specify the symbolic names for each of the range values the variable can take on, one at a time. These typically correspond to grounded PDDL facts, except for values that represent that none out a set of PDDL facts is true.
        ...
        for i, line in enumerate(sas_lines):
            if i > 0 and "end_metric" in sas_lines[i-1]:
                ...
                if count_variables == num_variables:
                    ...
                    new_sas_file.append(f"Atom not-causes-failure({obj.name.lower()})\n")
                    new_sas_file.append(f"NegatedAtom not-causes-failure({obj.name.lower()})\n")
        ...
        # TODO Fix Mutex and copy others
        num_operators = None
        ...
        # The third line contains a single number, denoting the number of prevail conditions.
        # The following lines describe the prevail conditions, one line for each condition. A prevail condition is given by two numbers separated by spaces, denoting a variable/value pairing in the same notation for goals described above.
        # The first line after the prevail conditions contains a single number, denoting the number of effects.
        ...
        for i, line in enumerate(sas_lines[sas_file_i:]):
            if "begin_operator" in line:
                ...
                num_prevail_conditons = int(sas_lines[sas_file_i+i+2].replace("\n",""))
                # list of preconditons
                num_effects = int(sas_lines[sas_file_i+i+3+num_prevail_conditons].replace("\n",""))
                # list of effects
                cost = int(sas_lines[sas_file_i+i+4+num_prevail_conditons+num_effects].replace("\n",""))
                end_operator_str = sas_lines[sas_file_i+i+5+num_prevail_conditons+num_effects]
                ...
                if operator_str.replace("\n","") == ground_op_str:
                    ...
                    new_sas_file.append(f"{num_variables} 0\n") # additional precondition
                else:
                ...
                # append effects
                if obj.name.lower() in operator_str and "navigateto" not in operator_str:
                    new_sas_file.append(f"{num_effects+1}\n")
                    new_sas_file.append(f"0 {num_variables} -1 0\n") # additional effect
                else:
                    new_sas_file.append(f"{num_effects}\n")
                for j in range(num_effects):
                    new_sas_file.append(sas_lines[sas_file_i+i+4+num_prevail_conditons+j])
...
def _sesame_plan_with_fast_downward(
    ...
    while True:
        ...
        # TODO fix number of skeletons optimized
        metrics["num_skeletons_optimized"] = 1
