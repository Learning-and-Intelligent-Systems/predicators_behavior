...
import logging
from typing import List, Set, Union, Tuple, Callable, Any
...
from predicators.structs import ExplorationStrategy, State, Action
...
class PartialBilevelPlanningExplorer(BilevelPlanningExplorer):
    ...
    def get_exploration_strategy(self, train_task_idx: int,
                                 timeout: int) -> Union[
        tuple[Callable[[State], Action], Callable[[State], bool]], tuple[
            Callable[[State], Action], Callable[[Any], bool], Any]]:
        logging.info(len(self._train_tasks))
        logging.info(train_task_idx)
