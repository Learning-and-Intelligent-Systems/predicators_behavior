...
def create_toggle_on_option_model(
    plan: List[List[float]], _original_orientation: List[List[float]],
    obj_to_toggled_on: "URDFObject") -> Callable[[State, "BehaviorEnv"], None]:
    del plan

    def toggleOnObjectOptionModel(_init_state: State, env: "BehaviorEnv") -> None:
        logging.info(f"PRIMITIVE: Attempting to toggle on {obj_to_toggled_on.name}")
        if np.linalg.norm(
                np.array(obj_to_toggled_on.get_position()) -
                np.array(env.robots[0].get_position())) < 2:
            if hasattr(obj_to_toggled_on, "states") and object_states.ToggledOn in obj_to_toggled_on.states:
                obj_to_toggled_on.states[object_states.ToggledOn].set_value(True)
            else:
